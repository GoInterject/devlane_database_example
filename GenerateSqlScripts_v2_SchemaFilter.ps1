...
{
    if($StoredProcedure)      {$Objects += $db.StoredProcedures     | Where-Object {!($_.IsSystemObject) -and ($_.DateLastModified -ge $ModifiedDate -or $ModifiedDate -eq $null) -and ($_.schema -eq $schemafilter)}}
    if($View)                 {$Objects += $db.Views                | Where-Object {!($_.IsSystemObject) -and ($_.DateLastModified -ge $ModifiedDate -or $ModifiedDate -eq $null) -and ($_.schema -eq $schemafilter)}}
    if($Table)                {$Objects += $db.Tables               | Where-Object {!($_.IsSystemObject) -and ($_.DateLastModified -ge $ModifiedDate -or $ModifiedDate -eq $null) -and ($_.schema -eq $schemafilter)}}
    if($Role)                 {$Objects += $db.Roles                | Where-Object {!($_.IsSystemObject) -and ($_.DateLastModified -ge $ModifiedDate -or $ModifiedDate -eq $null) -and ($_.schema -eq $schemafilter)}}
    if($Rule)                 {$Objects += $db.Rules                | Where-Object {!($_.IsSystemObject) -and ($_.DateLastModified -ge $ModifiedDate -or $ModifiedDate -eq $null) -and ($_.schema -eq $schemafilter)}}
    if($Schema)               {$Objects += $db.Schemas              | Where-Object {!($_.IsSystemObject) -and ($_.DateLastModified -ge $ModifiedDate -or $ModifiedDate -eq $null) -and ($_.schema -eq $schemafilter)}}
    if($UserDefinedFunction)  {$Objects += $db.UserDefinedFunctions | Where-Object {!($_.IsSystemObject) -and ($_.DateLastModified -ge $ModifiedDate -or $ModifiedDate -eq $null) -and ($_.schema -eq $schemafilter)}}
    if($User)                 {$Objects += $db.Users                | Where-Object {!($_.IsSystemObject) -and ($_.DateLastModified -ge $ModifiedDate -or $ModifiedDate -eq $null) -and ($_.schema -eq $schemafilter)}}
}

if ($ObjectSearchEnabled -eq $True)
{
    $ObjectListPath = ".\PowershellScripts\ScriptOutDatabase_ObjectList.txt"
